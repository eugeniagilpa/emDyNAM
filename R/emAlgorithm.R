...
softEMAlgorithm <- function(net0, net1, theta0, beta0, formula, nmax = 10) {
  ...
  return(list(
    "logLik" = logLik, "beta" = beta, "se" = se, "betaCrea" = betaCreaDF, "betaDel" = betaDelDF, "index" = index,
    "diff" = diff
  ))
}
softEMAlgorithmMC <- function(nmax, net0, net1, theta0, beta0, formula, num_cores = 1) {
  ...
  while (diff > 1e-3) {
    ...
    logLik <- clusterApply(cl, seq_along(splitIndicesPerCore), logLikelihoodMC,
      permut = permut,
  ...
  return(list(
    "logLik" = logLik, "beta" = beta, "se" = se, "index" = index,
